...
$WI.Class.Selectbox = new $WI.Class({
	Create: function(options) {			
		...
		this.AddEvent({obj: document.body, type: 'click', onevent: {obj: this, fire: 'closebox'}});
	...
	AddOption: function(options) {
		if(!options||(!options.text&&!options.value)) { //create separator
			...
			row.checkbox = this._insertDOM(row, {objType: 'div', objClass: 'element-selectbox-row-checkbox'}, 'insertinto');
			//set icon
			if(options.icon) 
		...
		this._cancelSelect(null, true, row);
		return row;
	},	
	...
	_onBoxEvent: function(event, _target, obj) {		
		if(event.fire=='openbox') {			
			...
			this._visibility(this.obj.drop, false);			
			this._ajustDivs();
			this._visibility(this.obj.drop, true);	
