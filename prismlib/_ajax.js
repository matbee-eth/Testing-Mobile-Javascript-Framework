...
$WI.Class.Ajax = new $WI.Class({
	...
	_Request: function(options) {
		...
		if((___cache = this._checkCache()) == -1) ___cache = this._setCache(null, null);
		else if(!this._getCache(___cache).result) return false; 	
		//check cache, if result has been already set
		if(this.options.cache&&___cache!=-1&&$WI.GLOBAL_AJAX_REGISTRY[___cache].result) {
...
$WI.Class.XmlDocument = new $WI.Class({
	...
	getNodeValue: function(xpath) {
		return this.getNode(xpath).getNodeValue().replace(/&lt;\!\[CDATA\[/g, "<![CDATA[").replace(/]]&gt;/g, "]]>");
	},
...
$WI.Class.XmlNode = new $WI.Class({
	...
	getLastChild: function() {
		return new $WI.Class.XmlNode().Init(this.xmlDoc, this.xmlNode.lastChild);
	},	
	//returns parent node
	getParentNode: function() {
		return new $WI.Class.XmlNode().Init(this.xmlDoc, this.xmlNode.parentNode);
	},	
	//returns a value for an attribute
	getAttribute: function(attribute) {
		if(this.xmlNode.getAttribute(attribute)) 			
